...
IDEMtools was developed to calculate fish and benthic macroinvertebrate metrics and Indices of Biotic Integrity (IBI) for wadeable and boatable, freshwater, perennial streams in Indiana. Detailed documentation of IBI development can be downloaded [here](https://github.com/Blocktt/IDEMtools/tree/main/reports). Users can run any data through the IBI calculator and get a result. However, if samples do not meet the criteria listed on the Background page, results should be interpreted with caution.
...
## Foreword
Tetra Tech, Inc. developed the IDEMtools shiny app for the Indiana Department of Environmental Management (IDEM). It allows IDEM and their partners to calculate IBI scores for fish and benthic macroinvertebrate samples. The new IBIs will improve IDEM’s diagnostic ability to identify degradation in biological integrity and water quality. The app is streamlined and easy to operate, and only requires an input dataset to function. Shiny apps are interactive web applications that serve as graphical user interfaces. They are linked to R, which is an open source programming language and software environment for statistical computing.
...
Click here to view an [EXAMPLE INPUT FILE](https://github.com/Blocktt/IDEMtools/tree/main/inst/shiny-examples/MassIBItools/Examples).
...
* Check the taxa names in your input file against the ‘TAXAID’ field in the [TaxaTranslator table](https://github.com/Blocktt/IDEMtools/tree/main/Data_Prep). Ideally the names match (but this is not required). The TAXAID field is included in the Taxa Attribute tables, which have trait assignments, phylogeny and other fields required in your input file. Contact Jen Stamp (Jen.Stamp@tetratech.com) if you need assistance with checking your taxa list and using the Taxa Attribute tables.
...
* Check the total number of individuals. Perform random subsampling if needed.
* Use NonTarget taxa designations and trait assignments (FFG, tolerance value, life cycle) that match with those in the appropriate  [TaxaAttribute tables](https://github.com/Blocktt/IDEMtools/tree/main/Data_Prep).
* Mark redundant (nondistinct) taxa in each sample in a way that is consistent with the criteria described in the [ExcludeDecisionCriteria](https://github.com/Blocktt/IDEMtools/tree/main/Data_Prep) document.
* Make sure you select the correct IBI for each sample and enter it into the ‘INDEX_REGION’ field. There are currently eight options: Bugs_N, Bugs_NC, Bugs_SW, Bugs_SE, Fish_WS, Fish_WL, Fish_BL, Fish_BH.
...
1. Load file
    + An example input file can be downloaded from Github [EXAMPLE LINK](https://github.com/Blocktt/IDEMtools/tree/main/inst/shiny-examples/MassIBItools/Examples)
